...
class ConvLSTMCell(nn.Module):
    ...
    def forward(self, input_tensor, cur_state):
        ...
        output = self.conv1x1(h_next)
        return output, (h_next, c_next)

    def init_hidden(self, batch_size):
        return (
            Variable(torch.zeros(
                batch_size, self.hidden_dim, self.height, self.width)).cuda(),
            Variable(torch.zeros(
                batch_size, self.hidden_dim, self.height, self.width)).cuda()
        )
...
class ConvLSTMDecoder(nn.Module):
    ...
    def forward(self, input_tensor, hidden_state, target):
        """
    ...
    def _check_kernel_size_consistency(kernel_size):
        ...
        (isinstance(kernel_size, list) and all([isinstance(elem, tuple) for elem in kernel_size]))):  # noqa
