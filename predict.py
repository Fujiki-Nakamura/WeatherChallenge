...
DATA_DIR = '../inputs/hkl/'
WEIGHTS_DIR = '../logs.prednet/{}/'.format(EXP_ID)
RESULTS_SAVE_DIR = '../logs.prednet/{}/'.format(EXP_ID)
INPUT_PATH = os.path.join(DATA_DIR, 'X_2018_50x96x168x128x1.hkl')
TARGET_PATH = os.path.join(DATA_DIR, 'X_2018_50x96x672x512x1.hkl')
...
def main():
    ...
    if is_making_submission:
        ...
        # MAE
        MAE = np.mean(np.abs(
            crop_eval_area(y_test) - crop_eval_area(resize(X_hat[:, input_ts:]))))
        print('MAE/Valid {:.4f}'.format(MAE))
        ...
        plot_save_dir = os.path.join(
            RESULTS_SAVE_DIR, 'plots_{}ts-{}ext/'.format(nt, extrap_start_time))
