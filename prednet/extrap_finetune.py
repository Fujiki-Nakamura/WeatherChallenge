...
nt = 50
nextrap = 20
extrap_start_time = nt - nextrap  # starting at this time step, the prediction from the previous time step will be treated as the actual input
...
extrap_weights_file = os.path.join(
    WEIGHTS_DIR, 'prednet_kitti_weights-extrapfinetuned-{}-{}.hdf5'.format(nt, nextrap))  # where new weights will be saved
extrap_json_file = os.path.join(
    WEIGHTS_DIR, 'prednet_kitti_model-extrapfinetuned-{}-{}.json'.format(nt, nextrap))
